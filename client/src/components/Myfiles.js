...
class Myfiles extends Component {
  ...
  render() {
    ...
    // console.log(Object.keys(sharedFiles));
    return (
      <div>
        <div className="file-section">
          ...
          {!!sharedFiles && <SharedFiles files={sharedFiles} user={user} />}
        </div>
...
const SharedFiles = ({ files ,user }) =>
<div>
  <h3>Shared Files</h3>
    
  {Object.keys(files).map(key =>
    <div className="file-link" key={key}>{files[key].filename}<button data-user={files[key].owner} id={files[key].filename} onClick={downloadFile}>Download</button></div>
  )}
</div>  
